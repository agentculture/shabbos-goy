...
shabbos-goy vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. shabbos-goy
...
#   - consumer-identifying prose: `guildmaster` → `shabbos-goy` (NOT
...
If a re-sync would lose a shabbos-goy adaptation, lift the change
...
#  `which shabbos-goy vendors from guildmaster`).
