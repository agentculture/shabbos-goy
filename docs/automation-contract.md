...
harness in a clone of `shabbos-goy`, and is the human-readable
...
```sh
claude -p "Answer with exactly one word, yes or no: does CLAUDE.md describe this project as shabbos-goy?"
```
...
```sh
qwen --approval-mode plan --output-format text "Answer with exactly one word, yes or no: does QWEN.md describe this project as shabbos-goy?"
```
...
|---|---|---|
| `claude -p "…shabbos-goy?"` | `Yes` | `?? docs/harness-invocations.yaml` (pre-existing; otherwise empty) |
| `pi -p --approve --no-tools "…lobes-cli?"` | `yes` | `?? docs/harness-invocations.yaml` (pre-existing; otherwise empty) |
| `qwen --approval-mode plan --output-format text "…shabbos-goy?"` | `yes` | `?? docs/harness-invocations.yaml` (pre-existing; otherwise empty) |
