...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope shabbos-goy
...
scope:** the default personal scope is **private** (`--scope shabbos-goy
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `shabbos-goy`). Private records are served only to a recall in the same
